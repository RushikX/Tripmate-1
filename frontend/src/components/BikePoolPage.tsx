...
import { Bike, MapPin, Clock, Star, ArrowLeft, Plus, Users, User, Calendar } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useToast } from '@/hooks/use-toast';
import UserProfileDialog from './UserProfileDialog';

interface BikePoolTrip {
  _id: string;
...
const BikePoolPage = () => {
  const { user } = useAuth();
  const [trips, setTrips] = useState<BikePoolTrip[]>([]);
  const [myTrips, setMyTrips] = useState<BikePoolTrip[]>([]);
  const [myBookings, setMyBookings] = useState<any[]>([]);
  const [searchFrom, setSearchFrom] = useState('');
  const [searchTo, setSearchTo] = useState('');
  const [loading, setLoading] = useState(false);
  const [selectedTrip, setSelectedTrip] = useState<BikePoolTrip | null>(null);
  ...
  const [editingTrip, setEditingTrip] = useState<BikePoolTrip | null>(null);
  ...
  const fetchTrips = async () => {
    setLoading(true);
    try {
      const response = await api.get<{ trips: BikePoolTrip[], pagination: any }>(API_ENDPOINTS.BIKE_POOL.GET_ALL);
  ...
  const fetchMyTrips = async () => {
    ...
    try {
      const data = await api.get<BikePoolTrip[]>(API_ENDPOINTS.BIKE_POOL.DRIVER_TRIPS);
  ...
  const fetchMyBookings = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const bikePoolBookings = data.filter(booking => booking.type === 'bike-pool');
        setMyBookings(bikePoolBookings);
      } else {
        setMyBookings([]);
      }
    } catch (error) {
  ...
  const handleBooking = async () => {
    ...
    if (!bookingForm.pickupTime || !bookingForm.pickupLocation) {
      toast({
        title: "Error",
    ...
    try {
      const startTime = new Date(bookingForm.pickupTime);
      const endTime = new Date(selectedTrip.reachTime);
  ...
  const handleCreateTrip = () => {
    navigate('/create-bike-pool');
  };

  const handleEditTrip = (trip: BikePoolTrip) => {
    setEditingTrip(trip);
    setEditForm({
      ...
      bikeBrand: trip.bikeDetails.brand,
      bikeModel: trip.bikeDetails.model,
      bikeColor: trip.bikeDetails.color,
      bikeNumberPlate: trip.bikeDetails.numberPlate,
      bikeYear: trip.bikeDetails.year || new Date().getFullYear(),
      bikeType: trip.bikeDetails.bikeType || 'motorcycle',
  ...
  const handleUpdateTrip = async () => {
    ...
    try {
      const updatedData = {
        ...
        bikeDetails: {
          brand: editForm.bikeBrand,
          model: editForm.bikeModel,
          color: editForm.bikeColor,
          numberPlate: editForm.bikeNumberPlate,
          year: editForm.bikeYear,
          bikeType: editForm.bikeType
        },
      ...
      await api.put(API_ENDPOINTS.BIKE_POOL.UPDATE(editingTrip._id), updatedData);
  ...
  const handleCancelTrip = async (tripId: string) => {
    ...
    setLoading(true);
    try {
      await api.put(API_ENDPOINTS.BIKE_POOL.UPDATE(tripId), { status: 'cancelled' });
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-6xl bg-white dark:bg-black">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Bike Pooling</h1>
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <div className="text-sm text-gray-600 dark:text-gray-300">Bike Details</div>
                          <div className="font-medium text-gray-900 dark:text-white">
                            {trip.bikeDetails?.brand || 'Unknown'} {trip.bikeDetails?.model || 'Unknown'} - {trip.bikeDetails?.color || 'Unknown'}
                          </div>
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            {trip.bikeDetails?.numberPlate || 'N/A'}
                          </div>
                        </div>
                      </div>
                ...
                <Card key={booking._id} className="hover:shadow-md transition-shadow bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
                  <CardContent className="p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-3">
                      <div>
                        <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">
                          {booking.bikePoolTrip?.fromDestination} → {booking.bikePoolTrip?.toDestination}
                        </h3>
                        <p className="text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
                      <div>
                        ...
                        <div className="font-medium text-sm sm:text-base text-gray-900 dark:text-white">{booking.bikePoolTrip?.driver?.name || 'Unknown'}</div>
                      </div>
                      <div>
                        <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">Amount</div>
                        <div className="font-medium text-sm sm:text-base text-gray-900 dark:text-white">₹{booking.bikePoolTrip?.pricePerHead || 0}</div>
                      </div>
                      <div>
                        <div className="text-xs sm:text-sm text-gray-600 dark:text-gray-300">Trip Date</div>
                        <div className="font-medium text-xs sm:text-sm text-gray-900 dark:text-white">
                          {new Date(booking.bikePoolTrip?.startTime).toLocaleDateString()}
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="bg-white dark:bg-gray-900">
          <DialogHeader>
            <DialogTitle className="text-gray-900 dark:text-white">Book Bike Pool Trip</DialogTitle>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="bg-white dark:bg-gray-900">
          <DialogHeader>
            <DialogTitle className="text-gray-900 dark:text-white">Edit Bike Pool Trip</DialogTitle>
            <DialogDescription className="text-gray-700 dark:text-gray-300">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit-bike-brand" className="text-gray-700 dark:text-gray-300">Bike Brand</Label>
              <Input
                id="edit-bike-brand"
                value={editForm.bikeBrand}
                onChange={(e) => setEditForm({...editForm, bikeBrand: e.target.value})}
                className="bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
              />
            </div>
            <div>
              <Label htmlFor="edit-bike-model" className="text-gray-700 dark:text-gray-300">Bike Model</Label>
              <Input
                id="edit-bike-model"
            ...
            <div>
              <Label htmlFor="edit-bike-type" className="text-gray-700 dark:text-gray-300">Bike Type</Label>
              <Select value={editForm.bikeType} onValueChange={(value) => setEditForm({...editForm, bikeType: value})}>
            ...
            <div>
              <Label htmlFor="edit-bike-color" className="text-gray-700 dark:text-gray-300">Bike Color</Label>
              <Input
                id="edit-bike-color"
            ...
            <div>
              <Label htmlFor="edit-bike-number-plate" className="text-gray-700 dark:text-gray-300">Bike Number Plate</Label>
              <Input
                id="edit-bike-number-plate"
                value={editForm.bikeNumberPlate}
                onChange={(e) => setEditForm({...editForm, bikeNumberPlate: e.target.value})}
                className="bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
              />
            </div>
            <div>
              <Label htmlFor="edit-bike-year" className="text-gray-700 dark:text-gray-300">Bike Year</Label>
              <Input
                id="edit-bike-year"
                type="number"
                value={editForm.bikeYear}
                onChange={(e) => setEditForm({...editForm, bikeYear: parseInt(e.target.value, 10)})}
                className="bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
              />
            </div>
            <div>
