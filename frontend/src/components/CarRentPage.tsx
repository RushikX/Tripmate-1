...
const CarRentPage = () => {
  ...
  const fetchListings = async () => {
    setLoading(true);
    try {
      const data = await api.get<CarListing[]>(API_ENDPOINTS.VEHICLES.CAR_RENT);
      setListings(data);
    } catch (error) {
  ...
  const fetchMyListings = async () => {
    ...
    try {
      const data = await api.get<CarListing[]>(API_ENDPOINTS.VEHICLES.CAR_MY_LISTINGS);
  ...
  const fetchMyRentals = async () => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const carRentals = data.filter(booking => booking.type === 'car-rent');
        setMyRentals(carRentals);
      } else {
        setMyRentals([]);
      }
    } catch (error) {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Car rental booking created successfully!",
      });
      ...
      toast({
        title: "Error",
        description: "Failed to create car rental booking",
        variant: "destructive"
      });
    }
  };
  ...
  const handleCreateListing = () => {
    navigate('/create-vehicle?type=car');
  };
  ...
  const handleRemoveListing = async (listingId: string) => {
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "Vehicle listing removed successfully!",
      });
  ...
  const filteredListings = Array.isArray(listings) ? listings.filter(listing => {
    ...
    if (rentalType !== 'all') {
      if (rentalType === 'short_term' && listing.pricePerHour > 100) return false;
      if (rentalType === 'long_term' && listing.pricePerDay > 1000) return false;
    }
    return true;
  }) : [];
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-6xl bg-white dark:bg-black">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Car Rentals</h1>
          <p className="text-gray-600 dark:text-gray-300 mt-1">Find and rent cars from trusted owners</p>
        </div>
        <Button onClick={() => navigate('/create-vehicle?type=car')} className="w-full sm:w-auto">
      ...
      <Card className="mb-6 bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
        <CardHeader>
          <CardTitle className="text-gray-900 dark:text-white">Search Cars</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Tabs defaultValue="available" className="space-y-6">
        <TabsList className="grid w-full grid-cols-3 bg-gray-100 dark:bg-gray-800">
          <TabsTrigger value="available" className="data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 text-gray-700 dark:text-gray-300">Available Cars</TabsTrigger>
                ...
                <Card key={listing._id} className="hover:shadow-md transition-shadow bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
                  <CardContent className="p-6">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            {listing.color} • {listing.fuelType} • {listing.transmission}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          ...
                          <div className="font-medium text-gray-900 dark:text-white">{listing.location.city}, {listing.location.state}</div>
                          <div className="text-sm text-gray-500 dark:text-gray-400">{listing.location.address}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Seats</div>
                        <div className="font-medium text-gray-900 dark:text-white">{listing.seats}</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Mileage</div>
                        <div className="font-medium text-gray-900 dark:text-white">{listing.mileage} km</div>
                      </div>
                      <div>
        ...
        <TabsContent value="my-listings" className="space-y-4">
          {myListings.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">No car listings yet</div>
          ) : (
            <div className="grid gap-4">
                  ...
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="text-gray-600 dark:text-gray-300">{listing.location.city}, {listing.location.state}</p>
                      </div>
                      <Badge variant={listing.isAvailable ? 'default' : 'secondary'} className="bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200">
                        {listing.isAvailable ? 'Available' : 'Not Available'}
                      </Badge>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div>
                        ...
                        <div className="font-medium text-gray-900 dark:text-white">₹{listing.pricePerHour}</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Daily Rate</div>
                        <div className="font-medium text-gray-900 dark:text-white">₹{listing.pricePerDay}</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Rating</div>
                        <div className="font-medium text-gray-900 dark:text-white">{listing.rating.toFixed(1)} ⭐</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Total Rentals</div>
                        <div className="font-medium text-gray-900 dark:text-white">{listing.totalRatings}</div>
                      </div>
                    </div>
        ...
        <TabsContent value="my-rentals" className="space-y-4">
          {myRentals.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">No car rentals yet</div>
          ) : (
            <div className="grid gap-4">
              {myRentals.map((rental) => (
                <Card key={rental._id} className="hover:shadow-md transition-shadow bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                      ...
                      <Badge variant={rental.status === 'confirmed' ? 'default' : 'secondary'} className="bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200">
                        {rental.status}
                      </Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        ...
                        <div className="font-medium text-gray-900 dark:text-white">₹{rental.amount}</div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Pickup Time</div>
                        <div className="font-medium text-gray-900 dark:text-white">
                          {new Date(rental.startTime).toLocaleTimeString()}
                        </div>
                      </div>
                      <div>
                        <div className="text-sm text-gray-600 dark:text-gray-300">Return Time</div>
                        <div className="font-medium text-gray-900 dark:text-white">
        ...
        <DialogContent className="bg-white dark:bg-gray-900">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="from-date" className="text-gray-700 dark:text-gray-300">From Date</Label>
                <Input
                  id="from-date"
                  type="date"
              ...
              <div>
                <Label htmlFor="to-date" className="text-gray-700 dark:text-gray-300">To Date</Label>
                <Input
                  id="to-date"
                  type="date"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="from-time" className="text-gray-700 dark:text-gray-300">From Time</Label>
                <Input
                  id="from-time"
                  type="time"
              ...
              <div>
                <Label htmlFor="to-time" className="text-gray-700 dark:text-gray-300">To Time</Label>
                <Input
                  id="to-time"
                  type="time"
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="bg-white dark:bg-gray-900">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit-brand" className="text-gray-700 dark:text-gray-300">Brand</Label>
                <Input
                  id="edit-brand"
              ...
              <div>
                <Label htmlFor="edit-model" className="text-gray-700 dark:text-gray-300">Model</Label>
                <Input
                  id="edit-model"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit-year" className="text-gray-700 dark:text-gray-300">Year</Label>
                <Input
                  id="edit-year"
                  type="number"
                  value={editForm.year}
              ...
              <div>
                <Label htmlFor="edit-color" className="text-gray-700 dark:text-gray-300">Color</Label>
                <Input
                  id="edit-color"
            ...
            <div>
              <Label htmlFor="edit-number-plate" className="text-gray-700 dark:text-gray-300">Number Plate</Label>
              <Input
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="edit-price-hour" className="text-gray-700 dark:text-gray-300">Price Per Hour</Label>
                <Input
                  id="edit-price-hour"
                  type="number"
              ...
              <div>
                <Label htmlFor="edit-price-day" className="text-gray-700 dark:text-gray-300">Price Per Day</Label>
                <Input
                  id="edit-price-day"
                  type="number"
